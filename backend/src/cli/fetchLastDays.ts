...
async function main() {
  ...
  await fs.writeFile(
    path.join(discoveryDir, `last-${days}d.json`),
    JSON.stringify({ generatedAtMs: Date.now(), days, fromMs, toMs, from: new Date(fromMs).toISOString(), to: new Date(toMs).toISOString(), campaigns }, null, 2),
    "utf-8",
  );
