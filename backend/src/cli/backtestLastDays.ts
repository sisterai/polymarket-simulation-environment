...
import { GammaClient, extractUpDownTokenIds, type GammaMarket } from "../polymarket/gamma.js";
...
function parseMsMaybe(x: unknown): number | null {
  if (typeof x !== "string") return null;
  const ms = Date.parse(x);
  return Number.isFinite(ms) ? ms : null;
}

function isLikely5mUpDownMarket(m: GammaMarket, hint: string) {
  const slug = String(m.slug ?? "").toLowerCase().trim();
  if (hint.toLowerCase() === "5m") return /^btc-updown-5m-\d+$/.test(slug);
  if (hint.toLowerCase() === "15m") return /^btc-updown-15m-\d+$/.test(slug);
  return slug.includes("btc-updown") && slug.includes(`-${hint.toLowerCase()}-`);
}

function getConditionId(m: GammaMarket): string | null {
  const c = (m.conditionId as string | undefined) ?? ((m as any).condition_id as string | undefined);
  return c ? String(c) : null;
}

function startMsFromUpDownSlug(slug: string | undefined): number | null {
  if (!slug) return null;
...
function parseRangeArgToMs(input: string): number {
  ...
  if (!m) throw new Error(`Invalid datetime '${input}'. Use ISO (e.g. 2025-04-05T00:00:00Z) or YYYY-M-D-HH-mm-ss.`);
...
async function main() {
  ...
  if (!bot) throw new Error(`Unknown bot '${botName}'. Available: ${Object.keys(Bots).join(", ")}`);
  ...
  if (toMs <= fromMs) throw new Error(`Invalid range: to (${toMs}) must be > from (${fromMs}).`);

  // 1) Discover markets, then filter locally.
  const discovered: { conditionId: string; slug?: string; startMs: number; upTokenId?: string; downTokenId?: string }[] = [];
  const pageSize = 200;
  ...
  for (let offset = 0; offset < 20_000; offset += pageSize) {
    ...
    for (const m of res ?? []) {
      const startMs =
        startMsFromUpDownSlug(m.slug) ??
        parseMsMaybe(m.startDate) ??
        parseMsMaybe((m as any).start_date);
      if (startMs === null) continue;
      if (startMs >= fromMs && startMs < toMs) pageHasAnyInRange = true;
      if (startMs < fromMs || startMs >= toMs) continue;
      if (!isLikely5mUpDownMarket(m, config.MARKET_TYPE_HINT)) continue;
      const q = config.MARKET_SEARCH_QUERY.toLowerCase();
      const hay = `${m.slug ?? ""} ${(m.question ?? (m as any).title ?? "") as string}`.toLowerCase();
    ...
    if ((res?.length ?? 0) < pageSize) break;
    if (discovered.length >= maxCampaigns) break;
    if (!pageHasAnyInRange && res?.length) {
      const oldest = res
        .map((m) => startMsFromUpDownSlug(m.slug) ?? parseMsMaybe(m.startDate) ?? parseMsMaybe((m as any).start_date) ?? Infinity)
        .reduce((a, b) => Math.min(a, b), Infinity);
      if (oldest !== Infinity && oldest < fromMs) break;
    }
  }
  ...
  const summaries: CampaignBacktestSummary[] = [];
  for (let i = 0; i < campaigns.length; i++) {
    ...
    const sources: CampaignSources = { trades, btcRange };
  ...
  const csv = stringify(
    summaries.map((r) => [r.conditionId, r.slug ?? "", r.startMs, r.endMs, r.trades, r.fees, r.equity, r.pnl]),
    { header: true, columns: ["conditionId", "slug", "startMs", "endMs", "trades", "fees", "equity", "pnl"] },
  );
