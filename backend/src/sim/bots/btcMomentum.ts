...
function pushRing(ring: (number | null)[], v: number | null) {
  if (ring.length < RING) ring.push(v);
  else {
    ring.shift();
    ring.push(v);
  }
...
function spanDelta(ring: (number | null)[]): number | null {
 ...
 */
export const btcMomentumBot: TradingBot = {
  ...
  decide(row: CampaignRow, ctx): BotAction {
    ...
    if (outcome === null) return { type: "HOLD" };
