 ...
 */
export const exampleSituationBot: TradingBot = {
  name: "example-situation-v1",
  decide(row: CampaignRow, ctx): BotAction {
    const up = midUp(row);
    ...
    if (up === null || down === null) return { type: "HOLD" };
    ...
    let btcBias: "UP" | "DOWN" | "NEUTRAL" = "NEUTRAL";
    const btc = row.btc_usd;
    if (btc != null && st.prevBtc != null) {
      const dBtc = btc - st.prevBtc;
      if (dBtc > 1) btcBias = "UP";
      else if (dBtc < -1) btcBias = "DOWN";
    }
    if (btc != null) st.prevBtc = btc;

    const cash = ctx.position.cash;
    const minNotional = 5;
    if (cash < minNotional) return { type: "HOLD" };
    ...
    function sizeFromDelta(absDelta: number): number {
