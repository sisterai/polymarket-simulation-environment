...
import { buildCampaignRowsFromSources, discoverPinnedMarkets, fetchCampaignSources, makeCampaignMeta, } from "../sim/buildCampaign.js";
...
export function buildRouter(config) {
    ...
    router.post("/download", express.json(), async (req, res) => {
        ...
        const sources = await fetchCampaignSources({ conditionId: body.conditionId, startMs: body.startMs, durationSec: body.durationSec }, { dataApi, btc });
        ...
        const built = buildCampaignRowsFromSources({ conditionId: body.conditionId, startMs: body.startMs, durationSec: body.durationSec }, sources);
    ...
    router.get("/discover", async (_req, res) => {
        const markets = await discoverPinnedMarkets(config.PINNED_MARKET_SLUGS
    ...
    router.post("/backtest", express.json(), async (req, res) => {
        ...
        const bot = Bots[body.bot];
        if (!bot)
            return res.status(400).json({ error: `Unknown bot '${body.bot}'` });
        const p = (body.params ?? {});
