...
function outcomeForLeg(legIndex) {
 ...
 */
export const ladder053Bot = {
    name: "ladder-053-v1",
    decide(row, ctx) {
        const st = stateFor(ctx.campaignStartMs);
        const midU = midUp(row);
        const midD = midDown(row);
        if (midU !== null && midU < THRESHOLD)
            st.armedUp = true;
        if (midD !== null && midD < THRESHOLD)
            st.armedDown = true;
        const finalizePrev = () => {
            if (midU !== null)
                st.prevUp = midU;
...
//# sourceMappingURL=ladder053.js.map