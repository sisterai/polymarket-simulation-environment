...
async function resolveUpDownTokens(gamma, conditionId) {
    const m = await gamma.getMarketByConditionId(conditionId);
    const pair = extractUpDownTokenIds(m);
    if (!pair)
        throw new Error(`Market ${conditionId} missing clobTokenIds/outcomes`);
...
async function main() {
    ...
    if (!conditionId)
        throw new Error("Usage: npm run collect:live -- --conditionId <conditionId> [--seconds 300] [--startMs <ms>]");
    if (!Number.isFinite(startMsRaw))
        throw new Error("--startMs must be a number (ms)");
    ...
    try {
        ...
        for (let i = 0; i < seconds; i++) {
            ...
            if (up.bid === null && up.ask === null)
                up = bestBidAsk(await clob.getBook(upTokenId));
            if (down.bid === null && down.ask === null)
                down = bestBidAsk(await clob.getBook(downTokenId));
...
//# sourceMappingURL=collectLiveCampaign.js.map